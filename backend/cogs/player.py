...
class MusicCog(commands.Cog):
    def __init__(self, bot: commands.Bot, playlist_manager: GuildPlaylistsManager):
        self.bot = bot
    ...
    async def get_voice_client(self, interaction: Interaction) -> VoiceClient:
        """獲取語音客戶端"""
    ...
    async def test(self, interaction: Interaction):
        ...
        logger.info(f"Guild VC: {interaction.guild.voice_client.channel}")
        logger.info(f"Playlist VC: {playlist.voice_client.channel}")
        logger.info(f"Bot VC list: {[x.channel for x in self.bot.voice_clients]}")

        await interaction.response.send_message("test")

    # ---------------- 連接、斷線 ----------------
    @app_commands.command(name = 'join', description = "連接至語音頻道")
    async def join(self, interaction: Interaction):
        ...
        await interaction.response.send_message('連接至語音頻道')
    ...
    async def leave(self, interaction: Interaction):
        ...
        await interaction.response.send_message('已中斷連接')
    ...
    async def resume(self, interaction: Interaction):
        ...
        await interaction.response.send_message('▶️ 回復播放')
    ...
    @app_commands.describe(number = "指定曲目編號（第n首）")
    async def skipto(self, interaction: Interaction, number: int):
        if not await self.check_user_voice(interaction):
            return
        
        playlist = self.playlist_manager.get_playlist(interaction.guild_id, interaction.channel_id)
        if number < 1 or number > len(playlist.songs):
            await interaction.response.send_message("指定曲目不存在")
            return
    ...
    async def clear(self, interaction: Interaction):
        ...
        await interaction.response.send_message('🗑 已清空曲目')
    ...
    async def info(self, interaction: Interaction):
        ...
        if song:
            await interaction.response.send_message(f'🎶 正在播放 -> {song["title"]} <-')
    ...
    # ---------------- Web token ----------------
    @app_commands.command(name = "web", description = "生成 Web Token")
